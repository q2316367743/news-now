...
export class NewsInstanceForCoolApk extends AbsNewsInstance {
  ...
  private get_app_token() {
    ...
    const md5_s = window.preload.util.crypto.md5(
      window.preload.util.crypto.encodeBase64(s),
    );
  ...
  private load(html: string) {
    ...
    return parser.parseFromString(
      `<html lang="zh"><body>${html}</body></html>`,
      "text/html",
    );
  }
  ...
  async getOriginRecords(api: NewsApi): Promise<Array<NewsInstanceRecord>> {
    ...
    const response = await api.http.json<Res>({
      url,
    ...
    return r.data
      ...
      .map((i) => {
        return {
          ...
          title:
            i.editor_title ||
            this.load(i.message).body.textContent?.split("\n")[0] ||
            "",
