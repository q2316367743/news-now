...
import { randomColor } from "@/utils/lang";
      ...
      <div class={"px-4px"}>
        <Form data={data.value}>
          <FormItem label={"主题色"} labelAlign={"top"} name={"primaryColor"}>
            <ColorPicker
              v-model={data.value.primaryColor}
              recentColors={false}
          ...
          >
            <Input
              v-model={data.value.source}
              placeholder={"请输入网RSS链接"}
            />
          </FormItem>
          <FormItem
            label={"浏览器打开方式"}
          ...
          {data.value.browser === "custom" && (
            <>
              ...
              >
                <Textarea
                  v-model={data.value.browserUserAgent}
                  placeholder={"请输入网站链接"}
